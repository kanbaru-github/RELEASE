class Public::PostsController < ApplicationController
  ...
  def index
    ...
    # params[:id]を使うには情報が足りない
    # Relationshiopのmuted_idが足りない（current_customer）
    # メソッドが多くても1行にまとめるのは通信が早いため
    ...
    # ?
      ...
      @posts= all_posts.page(params[:page]).left_outer_joins(:cheers).where.not(customer_id: muted_customer.pluck(:muted_id))
                   .group('posts.id').select('posts.*, COUNT("cheers.*") AS cheer').order(cheer: :desc)
